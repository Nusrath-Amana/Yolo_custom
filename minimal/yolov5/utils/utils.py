...
def bbox_sa_diou(box1, box2, lambda1=0.5, eps=1e-7):
    """
    Shape-Aware DIoU v2 calculation between matched boxes.
    Box format: [x_center, y_center, width, height]
    """

    box2 = box2.t()  # Transpose box2 if it's in batch format (for vectorized ops)
    ...
    # Compute union area with epsilon to avoid division by zero
    union = w1 * h1 + w2 * h2 - inter + eps
    iou = inter / union  # IoU

    # Compute enclosing (convex) box dimensions
    cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)
    ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)
    c_diag = cw**2 + ch**2 + eps  # Squared diagonal length of enclosing box

    # Compute squared center distance between boxes
    rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2)**2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2)**2) / 4

    # DIoU calculation
    diou = iou - rho2 / c_diag

    # Shape-aware penalty: absolute difference in aspect ratios
    sct = torch.abs(torch.abs((w1 / (h1 + eps))) - torch.abs((w2 / (h2 + eps))))

    # Return Shape-Aware DIoU
    return diou - lambda1 * sct
...
def compute_loss(p, targets, model):  # predictions, targets, model
    device = p[0].device  # Get device from predictions
    
    # Initialize losses on correct device
    ...
    # Define criteria with proper device handling
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            #giou = bbox_iou(pbox.t(), tbox[i].to(device), x1y1x2y2=False, CIoU=True)
            giou = bbox_sa_diou(pbox.t(), tbox[i].to(device))
...
def build_targets(p, targets, model):
    ...
    device = targets.device  # Get device from targets
    ...
    at = torch.arange(na, device=device).view(na, 1).repeat(1, nt)  # anchor tensor

    style = 'rect4'
    for i in range(det.nl):
        anchors = det.anchors[i].to(device)  # Ensure anchors on correct device
        ...
        if nt:
            ...
            a, t = at[j], t.repeat(na, 1, 1)[j.to(device)]  # filter with device sync
...
def output_to_target(output, width, height):
    ...
    """
    # Convert output to CPU numpy array if it's a tensor
    if torch.is_tensor(output):
        output = output.detach().cpu().numpy()
    elif isinstance(output, (list, tuple)):
        output = [x.detach().cpu().numpy() if torch.is_tensor(x) else x for x in output]

    targets = []
    for i, o in enumerate(output):
        if o is not None and len(o) > 0:
            # Handle both single and multiple predictions
            preds = o.reshape(-1, 6) if len(o.shape) == 1 else o
    ...
    return np.array(targets) if targets else np.zeros((0, 7), dtype=np.float32)
